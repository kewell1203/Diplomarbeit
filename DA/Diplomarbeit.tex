...
\begin{proof}
  ...
  Secondly, by $H=\frac{1}{2}$, $\int_{\mathbb{R}} f^2\frac{du}{2} = \frac{1}{2}\int_{0}^{|t|} \mathop{du} = |t| < \infty$. By $H \neq \frac{1}{2}$, we deal it with Taylor expansion. As $u$ goes to $-\infty$ $f(u)^2 = -(H-\frac{1}{2})(-u)^{H-\frac{3}{2}} + \mathrm{o}(u)$. Where $\mathrm{o}(u)$ tends to zero when $u$ near around $-\infty$, because $H-\frac{3}{2} < 0$ and therefore $f(u)$ is square integrable around $-\infty$. As $u$ goes to $t$, $f(u) \propto \mathbbm{1}_{\{t\ge u\}}(t-u)^{H-\frac{1}{2}}$. Hence, $f(u)$ is also square integrable around $u=t$. It is clear $f(u)=0$ when $u$ at around $\infty$. Then it satisfies the condition $\int_{-\infty}^{\infty}f^2(u)\,\mathop{\frac{du}{2}} < \infty$.\\
...
\end{proof}
Remark that, if we take $H=\frac{1}{2}$, choosing a restriction of the integrand on $\mathbb{R}_+$, $(U_{\frac{1}{2}}(t))_{t\ge 0}$ is a Brownian motion. 	
\begin{lemma}
...
\end{proof}
We can also define $U_H(t)$ as It\`o integral by \ref{sec:fbm}, then is has the same mean and variance as defined by stable integral. Since $U_H(t)$ is Gaussian, both of two versions have the same distribution. Follwing properties remains true also by Ito integral version.
\begin{corollary}
...
%	&=& c(H)\mathrm{E}[\brkt{\int_{\mathbb{R}} [\mathbbm{1}_{\{t\ge u\}}\cdot(t-u)^{H-\frac{1}{2}}  - \mathbbm{1}_{\{s\ge u\}}\cdot(s-u)^{H-\frac{1}{2}} \,\mathop{U_H(u)}}^2]\nonumber\\
	&\overset{\text{Corollary \ref{sec:the2}}}{=}& c(H)\mathrm{E}[\int_{\mathbb{R}} \brkt{[\mathbbm{1}_{\{t\ge u\}}\cdot(t-u)^{H-\frac{1}{2}}  - \mathbbm{1}_{\{s\ge u\}}\cdot(s-u)^{H-\frac{1}{2}}}^2 \,\mathop{du}]\nonumber\\
...
\end{corollary}
\begin{proof}
    Assume that $s \ge u $. Because the joint distribution of $(U_H(s), U_H(u))^T$ is Gaussian, $(1, -1) \cdot (U_H(s), U_H(u))^T $ is Gaussian. In other words,  $U_H(t_k+\tau) - U_H(s_k+\tau) \sim U_H(t_k) - U_H(s_k) \sim \mathcal{N}(0, (t_k - s_k)^{2H})$ for $k \in \{1\dots d\}$. Corresponding to  (\ref{sec:mcf}), 
	\begin{eqnarray*}
	\mathrm{E}[i\sum_{k=1}^{d}\xi_k (U_H(t_k + \tau) - U_H(s_k + \tau))] = \mathrm{E} [i\sum_{k=1}^{d}\xi_k (U_H(t_k) - U_H(s_k))]
  \end{eqnarray*}
Hence, $(U_H(t))$ has stationary increments.\\
...
\end{corollary}
\begin{proof}
  Cf. \cite{nourdin}, p.72, Lemma 6.3. In Theorem \ref{sec:lmemory}, we have $\varsigma_{S_H}^2(\tau) \propto H^2(2H-1)^2\tau^{4H-4} $. The sum of it over the range of $\tau$ is finite if and only if, according to the same reason as in Theorem \ref{sec:lmemory}, $4H<3$. That means $H < \frac{3}{4}$.
\end{proof}
\subsection{FBM is not Semimartingale for $H\neq \frac{1}{2}$}
Let us have a look at our Integration representation for FBM, in the case of FBM with Hurst index $\frac{1}{2}$, it must be an ordinary Brownian motion. Except for this, we will show FBM is not a seimimartingal.
...
\begin{proposition}
  ...
  \begin{enumerate}[topsep=0pt, itemsep=-1ex, partopsep=1ex, parsep=1ex, label=(\roman*)]
	\item $H_{n+1}(u)=(n+1)H_n(u)$ and $H_{n+2}(u)=u\cdot H_{n+1}(u) - (n+1)H_n(u)$ for all $n\in\mathbb{N}_0, u\in\mathbb{R}$.
...
\begin{proof}
 ...
 \begin{equation*}
  ...
  \end{eqnarray*}
	...
	\begin{eqnarray*}
	&&\sum\limits_{k=0}^{n-1} |\varsigma_{S_H}(k)| \\
	&\propto& H(2H-1)\sum\limits_{k=0}^{n-1}j^{2H-2} \\
	&\propto& H(2H-1)n\cdot n^{2H-2} \\
	&\propto& n^{2H-1}, 
  \end{eqnarray*}
  then
	\begin{eqnarray*}
	  \frac{2\alpha}{n} \sum\limits_{k=0}^{n-1} |\varsigma_{S_H}(k)| \propto n^{2H-2}
	\end{eqnarray*}
	as k goes to infinity. This leads  $\mathrm{E}[\{\frac{1}{n}\sum\limits_{j=1}^n f(U_H(j) - U_H(j-1)) - \mathrm{E}[f(W)]\}^2]=0$ due to $n^{2H-2}\rightarrow 0$ for $0<H<1$ as $n \rightarrow \infty$.\\
...
\end{eqnarray}
where $(X_t)_{t\ge 0}$ is a stochastic process, $a, \gamma\in\mathbb{R}_{+}$ and $(U_H(t))_{t\ge 0} $ a FBM with Hurst exponent $H$. In fact, given a initial solution $X_0(\omega)=b(\omega)$, then it is
\begin{eqnarray}
...
\begin{proof}
  ...
  \end{eqnarray*}
  Notice that since $(U_H(t))$ is centered Gaussian process, any linear transform of its instances is centered Gaussian again. In other words, 
  \begin{eqnarray*}
	\sum_{j=1}^d \xi_j \brkt{\sum_{l=0}^{k_j-1} e^{au^j_l}(U_H(u^j_{l+1}) - U_H(u^j_{l})) }
  \end{eqnarray*}
  is centered Gaussian. Passing $\epsilon^1,\dots,\epsilon^d$ to zero, it implies immediately $(\hat{X}_{t_1},\dots, \hat{X}_{t_d})$ is centered Gaussian process due to the continuity theorem of characteristic function.\\
  On the other hand,
	\begin{eqnarray*}
	\mathrm{E}[\exp\{i\sum_{j=1}^d \xi_j\hat{x}_{t_j }\}] \approx \mathrm{E}[\exp\{ i\sum_{j=1}^d \xi_j\brkt{\sum_{l=0}^{k_j-1} e^{au^j_l}(U_H(u^j_{l+1}) - U_H(u^j_{l})) }\}]
  \end{eqnarray*}
...
\end{eqnarray*}
for $N \in \mathbb{N}_{0}, \tau\in\mathbb{R}$, $\gamma, a$ are constants in (\ref{sec:oup}).
  \label{sec:autocov}
\end{theorem}
\begin{proof}
...
\begin{proof}
  ...
  \end{eqnarray*}
  is obviously tend to zero as $\tau$ goes to infinity. And For $\sum_{\tau=1}^{\infty} \varsigma_{\hat{X}_{H}}(\tau)$, we only need to check the term of $k=1$ in the summation, because in the case $k>1$, $\tau^{2H-2k} < \tau^{2H-2}$ for $\tau\in\mathbb{N}_{+}$. We deal with it in same way as in Theorem \ref{sec:lmemory}(use Cauchy condesation test). That means, for $2H < 1$, namely $H<\frac{1}{2}, \sum_{\tau=1}^{\infty} \varsigma_{X_{H}}(\tau)$ converges. For $H\in (\frac{1}{2}, 1)$, it diverges. Thus $(\varsigma_{\hat{X}_{H}}(t))_{t\ge 0}$ has long memory for $H\in(\frac{1}{2}, 1)$.
\end{proof}
\newpage
...
\begin{theorem}
  ...
  \end{eqnarray*}
  where $t\in[0, T], \{f_k\}_0^{n-1}$ is family of  $\mathcal{F}^{U_H}_k $-measurable function, $0 = \tau_1 < \cdots <\tau_n = T$ are stopping times with respect to $\mathcal{F}^{U_H}_t $ respectively,  with $\tau_{k+1} - \tau_k\ge m$ for some $m>0$ and $\mathcal{P}[f_k\neq 0]>0$ for $k\in\{0,\dots,n-1\}$, then
  \begin{eqnarray*}
...
\begin{definition}
  The \emph{fractional process} $(X_t)_t$ is defined as
  \begin{eqnarray}
...
\end{definition}
In \cite{comte}, Comte gave a truncated representation form of FBM $(U_{\alpha,t})$.
\begin{eqnarray}
  U_{\alpha,t} &=& \int_0^t \frac{(t-u)^\alpha}{\Gamma(1+\alpha)}\mathop{dB_u}, 
  \label{<sec:cc1>}
\end{eqnarray}
Where $\alpha \in (0, \frac{1}{2}), (B_u)$ Brownian motion. Notice that $\alpha$ is given by $H-\frac{1}{2}$ as in previous representation and the long memory property remains true.

\begin{proposition}
  Let $X(t)$ be a fractional process as by (\ref{sec:frac}). Then
  \begin{eqnarray}
...
\begin{proof}
  See \cite{core}, pp.20~21, equations (4), (5).
\end{proof}
\begin{definition}
  The \emph{fractional derivation of order $\alpha$} $X^{(\alpha)}_t $ is defined as
  \begin{eqnarray}
   && X^{(\alpha)}_t\\
...
\begin{proposition}
	$X^{(\alpha)}_t $ is well-defined and,
  \begin{enumerate}[topsep=0pt, itemsep=-1ex, partopsep=1ex, parsep=1ex, label=(\roman*)]
...
\end{proposition}
\begin{proof}
  See  \cite{core}, p.111, Proposition 4. and the Remark (i) of Proposition 4.
  \end{proof}
  \begin{corollary}
	\begin{eqnarray}
...
and 
\begin{eqnarray*}
  X_{\alpha,t} &=& \int_0^t \frac{(t-s)^\alpha}{\Gamma(1+\alpha)} \mathop{dX^{(\alpha)}(t)}.
\end{eqnarray*}
\end{example}
...
\end{eqnarray*}
	...
	Both in FSV and RFSV, there is a discretization motived by fractional derivation. An AR(1) approach can be applied in them. But %这个却不能在wsfv 应用。
	\\
